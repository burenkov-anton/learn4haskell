...
instance Functor (Secret e) where
    ...
    fmap f (Trap c) = Trap c
    fmap f (Reward d) = Reward (f d)
...
instance Applicative List where
    ...
    (<*>) Empty x = Empty
    (<*>) x Empty = Empty
    (<*>) (Cons f fx) (Cons x xs) = Cons (f x) (append (fmap f xs) (fx <*> (Cons x xs)))
...
andM x y = x >>= \a -> if a then y else pure False
--andM x y = pure (&&) <*> x <*> y
...
data Tree x = EmptyTree | Node (Tree x) x (Tree x) deriving (Show)
...
reverseTree (Node (left) x (right)) = Node (reverseTree right) x (reverseTree left)
